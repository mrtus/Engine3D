...
//		return (float)(help / 1.0) - (32 + 64 + 8);
	...
	private float[][] calculateHeights(int cx, int cy) {
		float[][] heightMap = new float[CHUNK_SIZE + SMOOTHING * 2][CHUNK_SIZE + SMOOTHING * 2];
		for (int x = -SMOOTHING; x < heightMap.length - SMOOTHING; x++) {
			for (int y = -SMOOTHING; y < heightMap.length - SMOOTHING; y++) {
				int xReal = cx + x;
				int yReal = cy + y;
	...
	private float[][] generateHeightMap(float[][] heightMap) {
		...
		for (int x = SMOOTHING; x < CHUNK_SIZE + SMOOTHING; x++) {
			for (int y = SMOOTHING; y < CHUNK_SIZE + SMOOTHING; y++) {
				float smoothHeight = (heightMap[x - 1][y - 1]
						+ heightMap[x - 1][y + 1]
						+ heightMap[x + 1][y - 1]
						+ heightMap[x + 1][y + 1]
						+ heightMap[x][y]
						...
						+ heightMap[x][y + 1])
						/ 9;
				smoothHeightMap[x - SMOOTHING][y - SMOOTHING] = smoothHeight;
			}
		}
...
//			alt4 &= 0xFFFF;
	...
	private float[][] smoothHeights(float[][] heightMap) {
		float[][] smoothHeightMap = heightMap.clone();
		for (int x = 1; x < heightMap.length - 1; x++) {
			for (int y = 1; y < heightMap.length - 1; y++) {
				float smoothHeight = (heightMap[x - 1][y - 1]
						...
						+ heightMap[x][y + 1])
						/ 9;
				smoothHeightMap[x - 1][y - 1] = smoothHeight;
			}
		}
		return smoothHeightMap;
	}
